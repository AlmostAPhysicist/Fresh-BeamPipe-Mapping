...
//      - Beamspot position (x, y) + errors # z is a made up number so it doesn't matter
...
private:
    ...
    Int_t nPV_;
    Float_t beamspot_x_, beamspot_y_;
    Float_t beamspot_xErr_, beamspot_yErr_;
    ...
    std::vector<float> vtx_chi2_, vtx_ndof_;
    ...
    std::vector<std::vector<float>> trk_dxy_beamspot_;  // dxy wrt beamspot
...
void ScoutingTreeMakerRun3::beginJob() {
    ...
    tree_->Branch("trk_dxy_beamspot", &trk_dxy_beamspot_);  // dxy wrt beamspot
...
void ScoutingTreeMakerRun3::clearVectors() {
    ...
    vtx_chi2_.clear(); vtx_ndof_.clear();
    vtx_ntracks_.clear();
    vtx_pvRegion_.clear();
    
    trk_pt_.clear(); trk_eta_.clear(); trk_phi_.clear();
    trk_dxy_origin_.clear(); trk_dxy_beamspot_.clear(); trk_dxyErr_.clear();
...
void ScoutingTreeMakerRun3::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    if (!haveBS) {
        LogWarning("ScoutingTreeMakerRun3") << "No valid beamspot. Skipping event.";
        return;
    }
    ...
    for (unsigned int t = 0; t < verticesH->size(); ++t) {
        ...
        // --- NEW: compute 2D dBV (vertex -> beamspot) and its uncertainty
        Measurement1D dBVref_meas = vertexDist2D.distance(v, bsVtx);
        ...
        std::vector<float> vtx_trk_dxy_origin, vtx_trk_dxy_beamspot, vtx_trk_dxyErr;
        std::vector<int> vtx_trk_nPixelHits, vtx_trk_nStripHits, vtx_trk_nTrackerLayers;

        const math::XYZPoint origin(0.,0.,0.);
        const math::XYZPoint bsPoint(bsVtx.x(), bsVtx.y(), bsVtx.z());
        for(auto track : tks) {
            ...
            vtx_trk_dxy_origin.push_back(track->dxy(origin));         // dxy wrt origin
            vtx_trk_dxy_beamspot.push_back(track->dxy(bsPoint));      // dxy wrt beamspot
        ...
        trk_dxy_beamspot_.push_back(std::move(vtx_trk_dxy_beamspot));
