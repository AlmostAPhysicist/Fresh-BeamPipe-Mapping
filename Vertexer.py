...
#         'file:///eos/user/a/amalhotr/DY2M_ScoutDefault.root'
#     )
# )
...
#                                 seed_tracks_src = cms.InputTag('hltScoutingUnpackProducer', 'Track'), # 'hltScoutingUnpackProducer', 'Track' -> ModuleLabel and Label(productInstanceName) for the type std::vector<reco::Track> Within RecreatedScoutUnpack.root
...
#     fileName = cms.untracked.string('/eos/user/a/amalhotr/DY2M_ScoutedVertexer.root'),
# )
...
process.Vertexer = cms.EDProducer('Vertexer',
                                ...
                                #   seed_tracks_src = cms.InputTag('displacedTracks'), # 'displacedTracks', '' -> ModuleLabel and Label(productInstanceName) for the type std::vector<reco::Track> Within RecreatedScoutUnpack.root
                                  #https://twiki.cern.ch/twiki/bin/view/CMSPublic/WorkBookTrackAnalysis#Track_information_in_MINIAOD
                                  #kvr_params = kvr_params,
                                  #do_track_refinement = cms.bool(False), # remove tracks + trim out tracks with IP significance larger than trackrefine_sigmacut and trackrefine_trimmax, respectively   
                                  resolve_split_vertices_loose = cms.bool(False), # an alternative merging routine with `loose` criteria, to merge any nearby vertices within a given dist or significance
